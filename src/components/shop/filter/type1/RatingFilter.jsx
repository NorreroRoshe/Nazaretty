  ...
  return (
    <>
      <span
        id="rating"
        className={
          selectedRating && selectedRating.length ? "item active" : "item"
        }
        onClick={() => setOpen(open === "rating" ? "" : "rating")}
      >
        {rating.name}
      </span>
      {open === "rating" ? (
        <div className="woocomerce__filtering-rate dropdowncommon">
          <span className="woocomerce__filtering-ftitle">{rating.name}</span>
